        ...
        <div className="text-sm">
            {Object.keys(grouped).sort().reverse().map(year => (
                <div key={year}>
                    ...
                    {expandedYears.has(year) && Object.keys(grouped[year]).sort().reverse().map(month => {
                        ...
                        return (
                            <div key={month} className="ml-4">
                                <button
                                    ...
                                    className="w-full text-left py-1 px-2 hover:bg-tertiary text-primary flex items-center"
                                >
                                    <span className="mr-1">{expandedMonths.has(yearMonth) ? '▼' : '▶'}</span>
                                    <Folder className='w-4 h-4 mr-1'/>
                                    {monthNames[parseInt(month)-1]}
                                </button>

                                {expandedMonths.has(yearMonth) && grouped[year][month].map(post => (
                                    <Link
                                        key={post.slug}
                                        href={`/posts/${post.slug}`}
                                        className="flex items-center py-1 px-2 ml-4 hover:bg-tertiary text-primary"
                                    >
                                        <FileText className='w-4 h-4 mr-1'/> {post.title}
                                    </Link>
                                ))}
                            </div>
                        );
